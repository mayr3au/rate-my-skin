...
export async function generateSkinReportPDF(data, lang = 'en') {
  ...
  const lifestyleItems = [
    ...
    { key: 'hygiene', icon: '✦', label: isFr ? 'Hygiène & Textile' : 'Hygiene & Habits', color: [141, 110, 99] },
    { key: 'sun', icon: '✦', label: isFr ? 'Exposition & UV' : 'Sun Exposure', color: [251, 192, 45] },
    { key: 'exercise', icon: '✦', label: isFr ? 'Activité & Sport' : 'Exercise', color: [30, 136, 229] },
    { key: 'temperature', icon: '✦', label: isFr ? 'Température Eau' : 'Water Temperature', color: [0, 172, 193] }
  ];
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(...COLOR_GOLD);
  doc.text('i', margin + 4, disclaimerY + 7);

  doc.setFont('helvetica', 'oblique');
  doc.setFontSize(7);
  doc.setTextColor(...COLOR_MEDIUM);
  const disclaimerText = isFr
    ? "Cette analyse est fournie uniquement à titre informatif et ne remplace pas un avis médical professionnel. Pour tout problème de peau persistant, veuillez consulter un dermatologue."
