...
export async function generateShareCanvas(data, lang = 'en') {
  // Ensure fonts are fully downloaded before drawing
  ...
  const statusLabel =
    score >= 78 ? (lang === 'fr' ? 'EXCELLENT' : 'EXCELLENT')
    : score >= 65 ? (lang === 'fr' ? 'BON' : 'GOOD')
  ...
  fillSpaced(ctx, lang === 'fr' ? 'ANALYSE DE PEAU PAR IA' : 'AI SKIN ANALYSIS', W / 2, 350, 6);
  ...
  fillSpaced(ctx, statusLabel, W / 2, 1038, 5);

  // ── Middle separator ───────────────────────────────────────────────────────
  drawGradientLine(ctx, W / 2 - 220, W / 2 + 220, 1080);

  // ── Key findings ───────────────────────────────────────────────────────────
  const sevColors = { mild: '#9CA3AF', moderate: '#F59E0B', significant: '#EF4444' };
  const sevLabels = {
    mild:        lang === 'fr' ? 'Léger' : 'Mild',
    moderate:    lang === 'fr' ? 'Modéré' : 'Moderate',
    significant: lang === 'fr' ? 'Significatif' : 'Significant',
  };

  if (problems.length > 0) {
    ctx.fillStyle = 'rgba(185,172,158,0.42)';
    ctx.font = '700 18px "DM Sans", Arial, sans-serif';
    fillSpaced(ctx, lang === 'fr' ? 'PROBLÈMES DÉTECTÉS' : 'KEY FINDINGS', W / 2, 1120, 5);
  ...
  ctx.textBaseline = 'alphabetic';
  ctx.fillText(
    lang === 'fr' ? 'Obtenez votre analyse gratuite' : 'Get your free skin analysis',
    W / 2, ctaY + 72
  );
