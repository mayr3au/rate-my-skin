...
export async function generateShareCanvas(data, lang = 'en') {
  ...
  // "ratemyskin.co" styled exactly like the logo at the top
  const urlFontSize = 58;
  const urlBaselineY = ctaY + 158;

  // Velvet gradient for "ratemy" / ".co"
  const urlVelvet = ctx.createLinearGradient(0, urlBaselineY - urlFontSize, 0, urlBaselineY + 8);
  urlVelvet.addColorStop(0,    '#3A2E26');
  urlVelvet.addColorStop(0.10, '#A87449');
  urlVelvet.addColorStop(0.50, '#D4A574');
  urlVelvet.addColorStop(0.90, '#A87449');
  urlVelvet.addColorStop(1,    '#3A2E26');

  // Measure widths
  ctx.font = `600 ${urlFontSize}px "DM Sans", Arial, sans-serif`;
  const ratemyWidth = ctx.measureText('ratemy').width;
  const skinWidth = ctx.measureText('skin').width;
  const dotCoWidth = ctx.measureText('.co').width;

  const totalUrlWidth = ratemyWidth + skinWidth + dotCoWidth;
  let currentX = W / 2 - totalUrlWidth / 2;

  // Draw "ratemy"
  ctx.font = `600 ${urlFontSize}px "DM Sans", Arial, sans-serif`;
  ctx.fillStyle = urlVelvet;
  ctx.textAlign = 'left';
  ctx.textBaseline = 'alphabetic';
  ctx.fillText('ratemy', currentX, urlBaselineY);
  currentX += ratemyWidth;

  // Draw "skin"
  ctx.font = `600 ${urlFontSize}px "DM Sans", Arial, sans-serif`;
  ctx.fillStyle = '#A87449';
  ctx.fillText('skin', currentX, urlBaselineY);
  currentX += skinWidth;

  // Draw ".co"
  ctx.font = `600 ${urlFontSize}px "DM Sans", Arial, sans-serif`;
  ctx.fillStyle = urlVelvet;
  ctx.fillText('.co', currentX, urlBaselineY);
