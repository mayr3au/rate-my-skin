...
export async function generateShareCanvas(data, lang = 'en') {
  ...
  // ── Background — site's cream gradient ─────────────────────────────────────
  const bg = ctx.createLinearGradient(0, 0, 0, H);
  bg.addColorStop(0,    '#FFFFFF');
  bg.addColorStop(0.40, '#FBF6F0');
  bg.addColorStop(0.75, '#EEDCD0');
  bg.addColorStop(1,    '#E3C9B5');
  ctx.fillStyle = bg;
  ctx.fillRect(0, 0, W, H);

  // Soft radial warmth behind the ring
  const ringCX = W / 2, ringCY = 820;
  const glow = ctx.createRadialGradient(ringCX, ringCY, 60, ringCX, ringCY, 580);
  glow.addColorStop(0, 'rgba(212,165,116,0.14)');
  glow.addColorStop(1, 'rgba(212,165,116,0)');
  ctx.fillStyle = glow;
  ctx.fillRect(0, 0, W, H);

  // ── Corner brackets ────────────────────────────────────────────────────────
  ctx.strokeStyle = 'rgba(168,116,73,0.22)';
  ctx.lineWidth = 2.5;
  ctx.lineCap = 'square';
  const B = 72, OFF = 72;
  const brackets = [
  ...
  // Favicon — very subtle, sits left of text
  if (favicon) {
    ctx.globalAlpha = 0.28;
    ctx.drawImage(favicon, logoX, logoBaselineY - faviconSize + 2, faviconSize, faviconSize);
    ctx.globalAlpha = 1;
  }
  ...
  ctx.fillStyle = 'rgba(168,116,73,0.48)';
  ...
  ctx.strokeStyle = 'rgba(168,116,73,0.12)';
  ctx.lineWidth = ringW;
  ctx.lineCap = 'butt';
  ctx.stroke();
  ...
  ctx.save();
  ctx.shadowColor = 'rgba(168,116,73,0.3)';
  ctx.shadowBlur = 24;
  ctx.beginPath();
  ...
  // "/ 100"
  ctx.fillStyle = 'rgba(168,116,73,0.55)';
