...
//
//   Bug 1 (SPF): spf slot previously used routine_step: 'sunscreen'.
//     The DB tags SPF products as routine_step = 'spf', NOT 'sunscreen'.
//     Fixed: spf slot now uses routine_step: 'spf'.
//
...
export function buildRoutineFilters(userConcerns, userSkinType) {
  ...
  return {
    morning: [
      ...
      {
        // ── BUG 1 FIX ──────────────────────────────────────────────────────
        // DB stores SPF products with routine_step = 'spf' (NOT 'sunscreen').
        // Using 'spf' here so findCandidatesForSlot matches correctly.
        // Skin-type filter is intentionally soft: SPF is returned even if no
        // exact skin_type match, because most SPF products are tagged 'all'.
        // ───────────────────────────────────────────────────────────────────
        slot: 'spf',
        stepText: 'Protéger avec un SPF50+',
        expectedRoutineStep: 'spf',
        filters: {
          routine_step: 'spf',
          time_of_day: ['morning', 'both'],
          skin_types: [normalizedSkinType, 'all'],
...
//
// For the `spf` slot and `oil_cleanser` slot the routine_step is now the exact
// DB value ('spf' / 'oil_cleanser'), so NO alias mapping is needed anymore.
//
...
export function deduplicateSlots(resolvedSlots) {
  ...
  // Collect morning product IDs
  (resolvedSlots.morning || []).forEach(slot => {
    (slot.candidates || []).forEach(c => morningProductIds.add(c.id));
  });
